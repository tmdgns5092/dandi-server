...
BaseService.addMethod(TaxinvoiceService.prototype, 'getInfos', function(CorpNum, KeyType, MgtKeyList, UserID, success, error){
  if(!KeyType || 0 === KeyType.length) {
    this._throwError(-99999999,'문서번호형태 입력되지 않았습니다.',error);
    return;
  }
...
BaseService.addMethod(TaxinvoiceService.prototype, "registRequest", function(CorpNum, Taxinvoice, Memo, UserID, success, error){
...
BaseService.addMethod(TaxinvoiceService.prototype, 'getViewURL', function(CorpNum, KeyType, MgtKey, UserID, success, error){
  if(!KeyType || 0 === KeyType.length) {
    this._throwError(-99999999,'문서번호유형이 입력되지 않았습니다.',error);
    return;
  }
...
BaseService.addMethod(TaxinvoiceService.prototype, 'getPDFURL', function(CorpNum, KeyType, MgtKey, UserID, success, error){
  if(!KeyType || 0 === KeyType.length) {
    this._throwError(-99999999,'문서번호유형이 입력되지 않았습니다.',error);
    return;
  }
...
BaseService.addMethod(TaxinvoiceService.prototype, 'getOldPrintURL', function(CorpNum, KeyType, MgtKey, UserID, success, error){
  if(!KeyType || 0 === KeyType.length) {
    this._throwError(-99999999,'문서번호유형이 입력되지 않았습니다.',error);
    return;
  }
...
BaseService.addMethod(TaxinvoiceService.prototype, 'getPDF', function(CorpNum, KeyType, MgtKey, UserID, success, error){
  if(!KeyType || 0 === KeyType.length) {
    this._throwError(-99999999,'문서번호유형이 입력되지 않았습니다.',error);
    return;
  }
...
BaseService.addMethod(TaxinvoiceService.prototype, "search", function(CorpNum, KeyType, DType, SDate, EDate, State, Type, TaxType, LateOnly, Order, Page, PerPage, TaxRegIDType, TaxRegIDYN, TaxRegID, QString, InterOPYN, UserID, IssueType, RegType, CloseDownState, MgtKey, success, error){
  ...
  if (RegType !== null) {
    targetURI += '&RegType=' + RegType.toString();
  }

  if (CloseDownState !== null) {
    targetURI += '&CloseDownState=' + CloseDownState.toString();
  }
  ...
  if (MgtKey !== "") {
    targetURI += '&MgtKey=' + encodeURIComponent(MgtKey);
  }
...
TaxinvoiceService.prototype.attachStatement = function(CorpNum, KeyType, MgtKey, SubItemCode, SubMgtKey, success, error){
  if(!MgtKey || 0 === MgtKey.length) {
    this._throwError(-99999999,'세금계산서 문서번호가 입력되지 않았습니다.',error);
    return;
  }
  ...
  if(!SubMgtKey || 0 === SubMgtKey.length) {
    this._throwError(-99999999,'첨부할 전자명세서 문서번호가 입력되지 않았습니다.',error);
    return;
  }
...
TaxinvoiceService.prototype.detachStatement = function(CorpNum, KeyType, MgtKey, SubItemCode, SubMgtKey, success, error){
  ...
  if(!SubMgtKey || 0 === SubMgtKey.length) {
    this._throwError(-99999999,'첨부해제할 전자명세서 문서번호가 입력되지 않았습니다.',error);
    return;
  }
...
TaxinvoiceService.prototype.getSealURL = function (CorpNum, UserID, callback, err) {
    this._executeAction({
        uri: '/?TG=SEAL',
        CorpNum: CorpNum,
        UserID: UserID,
...
TaxinvoiceService.prototype.getTaxCertURL = function (CorpNum, UserID, callback, err) {
    this._executeAction({
        uri: '/?TG=CERT',
        CorpNum: CorpNum,
        UserID: UserID,
